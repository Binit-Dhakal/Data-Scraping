...
class IndispidySpider(scrapy.Spider):
    name = 'indispidy'
    
    def start_requests(self):
        urls = []
        with open('/home/binit/python files/Python/CODINGNEST/cannaconnection/data.csv') as f:
            for line in f:
                urls.append(line)
    ...
    def parse(self, response):
        title = response.xpath('//div[contains(@class,"post-content")]/h1/text()').get()
        ...
        description = " "
        for r in response.xpath('//div[@class="rte"]/p/text()'):
            description += r.get()
